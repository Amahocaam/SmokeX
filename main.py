...
if not os.path.exists('./sessions'):
    os.mkdir('./sessions')
if not os.path.exists(f"Users/OWNER/phone.csv"):
   os.mkdir('./Users')
   os.mkdir(f'./Users/OWNER')
   open(f"Users/OWNER/phone.csv","w")
   ...
   if update_channel:
      try:
         ...
         if user.status == "kicked":
            await app.send_message(chat_id=message.chat.id,text="Üzgünüm efendim, Yasaklandınız. İletişim [Support](https://t.me/{update_channel}).", parse_mode="markdown", disable_web_page_preview=True)
            return 1
         ...
         await app.send_message(chat_id=message.chat.id, text="**Bir şeyler yanlış gitti. İletişim [Support](https://t.me/{update_channel}).**", parse_mode="markdown", disable_web_page_preview=True)
         return 1
 ...
 try:
   ...
   with open(f"Users/{message.from_user.id}/phone.csv", 'r')as f:
      ...
      if n<1 :
         ...
         return
      if a>100:
         await app.send_message(message.chat.id, "**Yalnızca {100-a} Telefon numarası ekleyebilirsiniz \n\nMade with ❤️**")
         return
      ...
      if not await client.is_user_authorized():
         ...
         try:
            ...
            try:
               ...
               return
      with open(f"Users/OWNER/phone.csv", 'r')as f:
         str_list = [row[0] for row in csv.reader(f)]
         NonLimited=[]
         for pphone in str_list:
            NonLimited.append(str(pphone))
         ...
         with open('1.csv', 'w', encoding='UTF-8') as writeFile:
            writer = csv.writer(writeFile, lineterminator="\n")
            writer.writerows(NonLimited)
         with open("1.csv") as infile, open(f"Users/OWNER/phone.csv", "w") as outfile:
            for line in infile:
                outfile.write(line.replace(",", ""))
      os.remove("1.csv")
